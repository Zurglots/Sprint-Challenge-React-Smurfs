...
server-side routing causes a new page to be rendered from the server. it has to re-render EVERYTHING not just the content you want.
client-side routing is faster and transitions more easily. but it needs to load the whole site on the first request so inital load time is longer.
